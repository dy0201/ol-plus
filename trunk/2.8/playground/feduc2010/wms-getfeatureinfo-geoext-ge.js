...
Ext.onReady(function() {	    		
    ...
    var fields = new OpenLayers.Layer.WMS(   
    	"Fields", 
    	"http://4682_OGC_AGS_0:8399/arcgis/services/ihs_petroleum/MapServer/WMSServer?",    	
		{ 																									
			layers: "fields",			
			styles: "",																																																																	
			srs: "EPSG:4326",											
    ...
    fields.setOpacity(0.49);								
    fields.setVisibility(true);

    var pipelines = new OpenLayers.Layer.WMS(   
    	"Pipelines", 
    	"http://4682_OGC_AGS_0:8399/arcgis/services/ihs_petroleum/MapServer/WMSServer?",    	
		{ 																									
			layers: "pipelines",			
    ...
    pipelines.setOpacity(1.0);								
    pipelines.setVisibility(true);
    ...
    wells.setOpacity(1.0);								
    wells.setVisibility(true);    	
	...
	var info_ctrl = new OpenLayers.Control.WMSGetFeatureInfo({
        url: 'http://4682_OGC_AGS_0:8399/arcgis/services/ihs_petroleum/MapServer/WMSServer?',
    ...
    var info_ctrl_video = new OpenLayers.Control.WMSGetFeatureInfo({
        ...
        vendorParams: {    	
    		version: "1.1.1",
    		xsl_template: "http://zeon/resources/xsl/getfeatureinfo/featureinfo_text_html_div_video_ihs.xsl"    		
    	},
    	handlerOptions: {
    	}
    });
    ...
    panel = new GeoExt.MapPanel({        
        ...
        region: "center",	//          
        map: map,        
        center: new OpenLayers.LonLat(48.01436, 29.11857),				// center the initial map
    ...
    registerRadio = function(node) {
	    if(!node.hasListener("radiochange")) {
	        node.on("radiochange", function(node) {
	        	...
	        	if(node.layer.name == "Wells") {
	        		info_ctrl.deactivate();
	        		info_ctrl_video.activate();
	        		info_ctrl_geojson.deactivate();
	        		OpenLayers.Console.debug("...identify on wells layer...");
	        	} else if(node.layer.name == "Fields") {
	        		info_ctrl.activate();
	        		info_ctrl_video.deactivate();
	        		info_ctrl_geojson.deactivate();
	        		OpenLayers.Console.debug("...identify on pipelines layer...");
	        	} else if(node.layer.name == "Pipelines") {
	        		info_ctrl.deactivate();
	        		info_ctrl_video.deactivate();
	        		info_ctrl_geojson.activate();
	        		OpenLayers.Console.debug("...identify on fields layer...");
	        	} else {
